...
{
  double s = 2. * Mel * E0;
  if (sqrt(s) < 2.*mChi) return 0.;   // A.C. e+e- -> A' -> chi chi can happen also for an A' and chi with large mass,
  ...
  sigma = sigma * (s - 4. / 3. * q * q); // A.C. this is for final state fermions (default)
  //sigma = sigma * (2./3. * q * q);     // A.C. this is for final state scalars
