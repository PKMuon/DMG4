...
{
  return ( "e-" == pDef.GetParticleName() || "mu-" == pDef.GetParticleName() );
}
    ...
    const G4Step & aStep)
{
  ...
  if(myDarkMatter->GetDMType() == 99) {
    ...
    if(myDarkMatter->GetParentPDGID() == 13) MRecoilLepton = Mel; // muon mode
    if(myDarkMatter->GetParentPDGID() == 11) MRecoilLepton = Mmu; // electron mode
